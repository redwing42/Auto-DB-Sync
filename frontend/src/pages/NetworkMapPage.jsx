...
import { MapContainer, TileLayer, Polyline, CircleMarker, Popup, Tooltip as LeafletTooltip, ZoomControl, useMap } from 'react-leaflet';
...
const NETWORK_COLORS = [
    '#2f81f7', '#3fb950', '#f0883e', '#a371f7', '#f85149',
    '#39d353', '#ffa657', '#79c0ff', '#ff7b72', '#d2a8ff'
];

function getNetworkColor(networkName, networkIndex) {
...
export default function NetworkMapPage() {
    ...
    const fetchData = async () => {
        try {
            ...
            const networks = [...new Set(res.routes.map(r => r.network_name).filter(Boolean))];
            const colorMap = {};
    ...
    const getColor = (route) => {
        const idx = networkColorMap.current[route.network_name] ?? 0;
        ...
        <div className="fade-in" style={{ height: 'calc(100vh - 80px)', display: 'flex', flexDirection: 'column' }}>
            <div className="page-header" style={{ flexShrink: 0, padding: '16px 24px' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '16px', fontSize: '12px', color: 'var(--text-secondary)' }}>
                    ...
                    <span>·</span>
                    <span>{data.pending_submissions.length} pending</span>
                </div>
            </div>
                        ...
                        return (
                            <React.Fragment key={`route-${route.id}`}>
                                <Polyline 
                                    ...
                                    eventHandlers={{
                                        click: () => setSelectedEntity({ type: 'route', ...route, _color: color }),
                                        mouseover: () => setHoveredRouteId(route.id),
                                        mouseout: () => setHoveredRouteId(null),
                                    }}
                                >
                                    <LeafletTooltip sticky>
                                        <strong>{route.start_lz_name || route.start_location_name} → {route.end_lz_name || route.end_location_name}</strong>
                                    </LeafletTooltip>
                                </Polyline>
                                {/* Source endpoint */}
                                <CircleMarker
